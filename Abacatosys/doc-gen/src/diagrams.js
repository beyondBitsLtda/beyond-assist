...
/* Paleta DELP (org). Barlow + Delp Red + cores de unidade. */
var COR = {
    red: '#CC0F10', redDark: '#A60C0D', redSoft: '#FDECEC',
    ink: '#1A1A1A', black: '#000000', muted: '#6B6B6B',
    line: '#E3DED7', bg: '#FFFFFF', panel: '#F4F1EC',
    subsea: '#2362D3', industria: '#FF6B05', servicos: '#0B861D', mooring: '#213D75',
    gray1: '#B0B0B0', gray2: '#EAE5DF'
};
var FONTE = "'Barlow','Inter',system-ui,-apple-system,sans-serif";
...
function caixa(x, y, w, h, opt) {
    opt = opt || {};
    var fill = opt.fill || COR.bg;
    var stroke = opt.stroke || COR.line;
...
function texto(x, y, s, opt) {
    ...
    return '<text x="' + x + '" y="' + y + '" font-family="' + FONTE + '" font-size="' + (opt.size || 12) +
        '" font-weight="' + (opt.weight || 500) + '" fill="' + (opt.fill || COR.ink) +
...
function caixaRotulada(x, y, w, h, rotulo, opt) {
    ...
    g += caixa(x, y, w, h, opt);
    if (opt.sub) {
        g += texto(x + w / 2, y + h / 2 - 1, visivel, { size: fs, weight: 600, anchor: 'middle', fill: opt.corTexto || COR.ink });
        g += texto(x + w / 2, y + h / 2 + 11, trunc(opt.sub, cabe(w, 8.5)), { size: 8.5, weight: 500, anchor: 'middle', fill: COR.muted });
    } else {
        g += texto(x + w / 2, y + h / 2 + fs / 2 - 1, visivel, { size: fs, weight: 600, anchor: 'middle', fill: opt.corTexto || COR.ink });
    }
    g += '</g>';
    return g;
}
...
function svg(w, h, corpo, opt) {
    ...
    var dim = opt.fixo
        ? 'width="' + w + '" height="' + h + '" style="max-width:none;background:' + COR.bg + '"'
        : 'width="100%" style="max-width:100%;height:auto;background:' + COR.bg + '"';
...
/* O view.file do application.info vem primeiro: e ele que o Fluig renderiza.
...
function camadasArquitetura(modelo) {
    ...
    if (temWidget) {
        add('apresentacao', 'Apresentacao da widget (FTL)', COR.gray1, nomesFtl(modelo));
        add('bootstrap', 'Bootstrap (SuperWidget)', COR.black, porPapel(modelo, ['bootstrap']));
        add('controller', 'Orquestracao (controller)', COR.ink, porPapel(modelo, ['controller']));
        add('dominio', 'Dominio, componentes e helpers', COR.red,
            porPapel(modelo, ['dominio', 'componentes', 'helper', 'infra']));
        add('libs', 'Bibliotecas de terceiros', COR.gray1, porPapel(modelo, ['lib']));
    }
    if (temForm) {
        add('cartao', 'Cartao do formulario (HTML)', COR.gray1, cartoesForm(modelo),
            temWidget ? 'entrada alternativa: o formulario do processo' : '');
        add('ev-exibe', 'Eventos que montam o cartao', COR.mooring, eventosForm(modelo, EV_EXIBE));
        add('ev-valida', 'Eventos de validacao / envio', COR.industria, eventosForm(modelo, EV_VALIDA));
        add('ev-outros', 'Outros scripts do cartao', COR.ink, eventosFormRestantes(modelo));
        add('aux-form', 'Scripts auxiliares do cartao', COR.ink, auxForm(modelo));
        add('css-form', 'Estilos do cartao (CSS)', COR.gray1, cssForm(modelo));
    }
    if (temWidget) {
        add('integracao', 'Integracao (integre*)', COR.subsea, porPapel(modelo, ['integracao']));
        add('acesso-dados', 'Acesso a dados client (ds*)', COR.servicos,
            porPapel(modelo, ['acesso-dados']).concat(nomesDatasetsClient(modelo)));
    } else {
        add('acesso-dados', 'Acesso a dados client (ds*)', COR.servicos, nomesDatasetsClient(modelo));
    }

    add('workflow', 'Eventos do processo (workflow, server-side)', COR.industria, nomesWorkflow(modelo));
    if (I.usaDatasetFactory) {
        add('factory', 'DatasetFactory / WKDataset (Fluig)', COR.mooring, ['DatasetFactory', 'WKDataset']);
    }
    add('dataset', 'Datasets server-side (Rhino)', COR.mooring, nomesDatasets(modelo));
    add('procedures', 'Stored procedures', COR.red,
        dedup(modelo.sql.procedures.map(function (p) { return p.nome; })));

    var ent = nomesEntidades(modelo);
    add('backend', 'Backend (SQL Server / TOTVS RM)', COR.industria,
...
function arquitetura(modelo) {
    ...
    camadas.forEach(function (c, i) {
        var h = alturas[i];
        yBanda.push({ y: y, h: h });
        corpo += '<rect x="0" y="' + y + '" width="' + W + '" height="' + h + '" rx="10" fill="' + COR.panel + '" stroke="' + COR.line + '"/>';
        ...
        corpo += texto(W - padX, y + 22, c.itens.length + (c.itens.length === 1 ? ' item' : ' itens'), { size: 10, anchor: 'end', fill: COR.muted });
        c.itens.forEach(function (it, k) {
            ...
            corpo += caixaRotulada(x, yy, bw, bh, it, {
                titulo: it, fill: COR.bg, stroke: c.cor, rx: 7, sw: 1.3, size: 10.5
            });
        });
        y += h + gap;
    });
    ...
    for (var i2 = 0; i2 < camadas.length - 1; i2++) {
        corpo += seta(W / 2, yBanda[i2].y + yBanda[i2].h, W / 2, yBanda[i2 + 1].y, { cor: COR.red, sw: 2 });
    }
...
function fluxograma(modelo) {
    ...
    if (temWidget) {
        passo('Usuario abre a widget', 'Fluig monta a pagina', 'inicio', COR.gray1);
        if (ftl.length) passo(resumo(ftl), 'markup da tela (FreeMarker)', 'arquivo', COR.gray1, ftl.join(', '));
        if (boot.length) passo(resumo(boot), 'bootstrap: SuperWidget.bindings', 'arquivo', COR.black, boot.join(', '));
        if (ctrl.length) passo(resumo(ctrl), 'orquestra a tela e os eventos', 'arquivo', COR.ink, ctrl.join(', '));
        if (integ.length) passo(resumo(integ), 'regra de integracao (' + integ.length + ')', 'arquivo', COR.subsea, integ.join(', '));
        if (dsCli.length) passo(resumo(dsCli), 'acesso a dados client (' + dsCli.length + ')', 'arquivo', COR.servicos, dsCli.join(', '));
    } else {
        passo('Usuario abre a tarefa', 'Fluig monta o cartao do processo', 'inicio', COR.gray1);
        if (cartoes.length) passo(resumo(cartoes), 'cartao do formulario (HTML)', 'arquivo', COR.gray1, cartoes.join(', '));
        if (evExibe.length) passo(resumo(evExibe), 'evento de exibicao do cartao (' + evExibe.length + ')', 'arquivo', COR.mooring, evExibe.join(', '));
        if (evOutros.length) passo(resumo(evOutros), 'demais scripts do cartao (' + evOutros.length + ')', 'arquivo', COR.ink, evOutros.join(', '));
        if (evValida.length) passo(resumo(evValida), 'validacao ao enviar (' + evValida.length + ')', 'arquivo', COR.industria, evValida.join(', '));
    }

    /* --------------------------------------------- cauda comum: dados */
    if (wf.length && !temWidget) {
        passo(resumo(wf), 'eventos do processo, server-side (' + wf.length + ')', 'servico', COR.industria, wf.join(', '));
    }
    if (I.usaDatasetFactory) {
        passo('DatasetFactory.getDataset()', 'ponte client → server do Fluig', 'servico', COR.mooring);
    }
    if (dsSrv.length) {
        passo(resumo(dsSrv), 'dataset server-side, Rhino (' + dsSrv.length + ')', 'arquivo', COR.mooring, dsSrv.join(', '));
    }
    if (dsSrv.length || modelo.sql.procedures.length) {
        passo('SQL / JDBC', 'datasource ' + (primeiroJndi(modelo) || 'JNDI da aplicacao'), 'dados', COR.red);
    }
    passo('SQL Server / TOTVS RM', modelo.dataModel.entidades.length + ' tabelas mapeadas', 'banco', COR.industria);
    ...
    for (var i = 0; i < passos.length - 1; i++) {
        ...
        corpo += seta(a.x + a.w / 2, a.y + a.h, b.x + b.w / 2, b.y - 2, { cor: COR.red, sw: 2 });
    }
    ...
    if (temWidget && temForm && (cartoes.length || evExibe.length || evValida.length || evOutros.length)) {
        if (cartoes.length) {
            blocos.push({ rot: 'Formularios (' + cartoes.length + ')', sub: 'cartao do processo', tit: cartoes.join(', '), cor: COR.industria });
        }
        var todosEv = evExibe.concat(evOutros).concat(evValida);
        if (todosEv.length) {
            blocos.push({ rot: 'Eventos do cartao (' + todosEv.length + ')', sub: 'displayFields, validacao…', tit: todosEv.join(', '), cor: COR.industria });
        }
    }
    if (temWidget && wf.length) {
        blocos.push({ rot: 'Eventos do processo (' + wf.length + ')', sub: 'beforeTaskSave, servicetask…', tit: wf.join(', '), cor: COR.industria });
    }
    if (blocos.length) {
        var yw = passos[2] ? passos[2].y : 60;
        corpo += texto(lx, yw - 12, 'ENTRADA PELO PROCESSO (BPMN)', { size: 10, weight: 700, fill: COR.industria });
        ...
        corpo += curva(lx + lw, ultimo.y + ultimo.h / 2, alvo.x, alvo.y + alvo.h / 2,
            { cor: COR.industria, sw: 1.6, dash: false, op: 0.9 });
    }

    /* --------------------------------------------- coluna direita */
    var grupos = [];
    if (temWidget) {
        if (deps.length) grupos.push({ rot: 'Dominio e componentes', sub: deps.length + ' modulos', tit: deps.join(', '), cor: COR.mooring });
        if (libs.length) grupos.push({ rot: 'Bibliotecas', sub: libs.length + ' arquivos', tit: libs.join(', '), cor: COR.gray1 });
        if (modelo.widget.css.length) {
            grupos.push({
                rot: 'Estilos (CSS)', sub: modelo.widget.css.length + ' folha(s)',
                tit: modelo.widget.css.map(function (c) { return c.nome; }).join(', '), cor: COR.gray1
            });
        }
    } else {
        if (aux.length) grupos.push({ rot: 'Scripts auxiliares', sub: aux.length + ' arquivos', tit: aux.join(', '), cor: COR.mooring });
        if (cssF.length) grupos.push({ rot: 'Estilos do cartao', sub: cssF.length + ' folha(s)', tit: cssF.join(', '), cor: COR.gray1 });
    }
    if (grupos.length) {
        var yd = passos[2] ? passos[2].y - 10 : 120;
        corpo += texto(rx, yd - 12, 'DEPENDENCIAS CARREGADAS', { size: 10, weight: 700, fill: COR.muted });
        ...
        grupos.forEach(function (g, k) {
            ...
            corpo += curva(rx, gy + 26, cx + cw, ancora.y + ancora.h / 2,
                { cor: COR.gray1, sw: 1.3, dash: true, op: 0.8 });
            H = Math.max(H, gy + 72);
        });
    }
...
function formaNo(x, y, w, h, rotulo, sub, forma, cor, titulo) {
    ...
    if (forma === 'inicio') {
        ...
        g += '<path d="M ' + x + ' ' + (y + ry) + ' A ' + (w / 2) + ' ' + ry + ' 0 0 1 ' + (x + w) + ' ' + (y + ry) +
            ...
            'fill="' + COR.bg + '" stroke="' + cor + '" stroke-width="1.8"/>';
        g += '<path d="M ' + x + ' ' + (y + ry) + ' A ' + (w / 2) + ' ' + ry + ' 0 0 0 ' + (x + w) + ' ' + (y + ry) + '" ' +
            'fill="none" stroke="' + cor + '" stroke-width="1.8"/>';
    } else if (forma === 'dados') {
        var sk = 16;
        g += '<path d="M ' + (x + sk) + ' ' + y + ' L ' + (x + w) + ' ' + y + ' L ' + (x + w - sk) + ' ' + (y + h) +
            ' L ' + x + ' ' + (y + h) + ' Z" fill="' + COR.bg + '" stroke="' + cor + '" stroke-width="1.6"/>';
    } else if (forma === 'servico') {
        g += '<rect x="' + x + '" y="' + y + '" width="' + w + '" height="' + h + '" rx="6" fill="' + COR.bg +
            '" stroke="' + cor + '" stroke-width="1.8" stroke-dasharray="6 3"/>';
    } else {
        g += '<rect x="' + x + '" y="' + y + '" width="' + w + '" height="' + h + '" rx="8" fill="' + COR.bg +
            '" stroke="' + cor + '" stroke-width="1.8"/>';
        g += '<rect x="' + x + '" y="' + y + '" width="4" height="' + h + '" rx="2" fill="' + cor + '"/>';
    }
    var corTxt = forma === 'inicio' ? '#fff' : COR.ink;
    var corSub = forma === 'inicio' ? 'rgba(255,255,255,.85)' : COR.muted;
    if (sub) {
...
function camadasCarga(modelo) {
    var grupos = [
        { role: 'lib', titulo: 'Bibliotecas', cor: COR.gray1 },
        { role: 'dominio', titulo: 'Dominio (POO)', cor: COR.red },
        { role: 'componentes', titulo: 'Componentes (UI)', cor: COR.mooring },
        { role: 'helper', titulo: 'Helpers', cor: COR.muted },
        { role: 'acesso-dados', titulo: 'Acesso a dados', cor: COR.servicos },
        { role: 'integracao', titulo: 'Integracao', cor: COR.subsea },
        { role: 'infra', titulo: 'Infra/Permissoes', cor: COR.industria },
        { role: 'controller', titulo: 'Controller', cor: COR.ink },
        { role: 'bootstrap', titulo: 'Bootstrap', cor: COR.black },
        { role: 'outro', titulo: 'Outros', cor: COR.gray1 }
    ];
    var porRole = {};
    ...
    linhas.forEach(function (g) {
        ...
        itens.forEach(function (it, k) {
            ...
            corpo += caixaRotulada(x, yy, bw, bh, rot, {
                titulo: it.ordem + '. ' + (it.path || it.nome), fill: COR.bg, stroke: g.cor, rx: 6, sw: 1.1, size: 9.5
            });
        });
...
function grafoChamadas(g) {
    ...
    bandas.forEach(function (b) {
        corpo += '<rect x="0" y="' + b.y + '" width="' + W + '" height="' + b.h + '" rx="10" fill="' + COR.panel +
            '" stroke="' + COR.line + '"/>';
        ...
        corpo += texto(W - padX, b.y + 18, b.itens.length + '', { size: 10, weight: 700, anchor: 'end', fill: COR.muted });
    });
    ...
    g.arestas.forEach(function (a, i) {
        ...
        if (!p || !q) return;
        var cor = COR_ARESTA[a.tipo] || COR.gray1;
    ...
    g.camadas.forEach(function (c) {
        (g.porCamada[c.id] || []).forEach(function (n) {
            ...
            corpo += caixaRotulada(p.x, p.y, p.w, p.h, n.rotulo, {
                titulo: titulo, sub: sub, fill: COR.bg, stroke: c.cor, rx: 7, sw: 1.3, size: 10,
