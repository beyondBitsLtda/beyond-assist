...
/**
 * POST /api/code-tasks/step   body: { taskId?, repo, baseBranch, instruction, filePaths? }
 *
 ...
 *
 * Sem `taskId`: cria uma tarefa nova e roda o 1º passo. Com `taskId`: retoma do passo salvo
 * em code_tasks.state. Ver runCodeTaskStep em src/lib/codeTasks.js.
 */
export async function POST(req) {
  const { taskId, repo, baseBranch, instruction, filePaths } = await req.json();
