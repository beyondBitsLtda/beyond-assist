...
/**
 * POST /api/radio/segment   body: { kind, category?, songTitle?, sourceText?, excludeKeys? }
 ...
 *
 * `excludeKeys` (só pra kind "talk") — chaves dos itens já mencionados recentemente NESSA
 * categoria (o cliente acumula isso entre chamadas, ver radioSeenKeysRef em assistant/page.js) —
 * evita repetir sempre os mesmos itens quando a categoria volta a ser sorteada. A resposta
 * devolve `usedKeys` com o que foi usado desta vez, pro cliente ir acumulando.
 */
export async function POST(req) {
  try {
    const { kind, category, songTitle, sourceText, excludeKeys } = await req.json();
    let text;
    let usedKeys = [];
    if (kind === "talk") {
      const data = await getCategoryData(category, excludeKeys || []);
      text = await generateRadioTalkSegment({ category, data: data.text });
    ...
    return jsonResponse({ ok: true, text, usedKeys });
  } catch (err) {
