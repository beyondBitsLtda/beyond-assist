...
export default function DashboardPage() {
  ...
  const sentinelStatusRows = sentinelSummary
    ? Object.entries(sentinelSummary.byStatus).map(([status, count]) => ({ key: status, label: status, values: [count] }))
    : [];
  ...
  return (
    <div style={{ padding: "24px 28px", height: "100%", overflowY: "auto" }}>
      ...
      {(topOverdue.length > 0 || topBreached.length > 0) && (
      ...
      <div className="bb-chart-grid">
        ...
        <HBarChart title="ATRASADAS · HÁ QUANTO TEMPO" rows={agingRows} series={agingSeries} />
        <HBarChart title="CARGA POR BOARD · NO PRAZO × ATRASADO × CONCLUÍDO" rows={boardsRows} series={boardsSeries} />
        <HBarChart title="QUARTO DE GUERRA · CARDS POR LISTA" rows={listRows} series={listSeries} />
        {sentinelSummary && (
          <>
            <HBarChart title="SENTINELA · CHAMADOS POR STATUS" rows={sentinelStatusRows} series={oneSeries} />
            <HBarChart title="SENTINELA · CHAMADOS POR PRIORIDADE" rows={sentinelPriorityRows} series={oneSeries} />
          </>
        )}
      </div>
