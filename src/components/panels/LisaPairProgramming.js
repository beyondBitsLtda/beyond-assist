...
//
// `fullscreen`: usado pela janela dedicada (src/app/pair/page.js), onde o plano vira uma coluna
// lateral e a IDE fica com todo o resto da tela. Sem ele, tudo empilha no painel do Modo
// Interativo, que é estreito.
...
export default function LisaPairProgramming({ onMood, fullscreen = false }) {
  ...
  // ---- escolha de repositório e nível (antes de a sessão existir) ----
  const setup = (
    <>
  ...
  // ---- o que ela decidiu: feature, motivo, branch e plano ----
  ...
  // ---- janela dedicada: plano numa coluna, IDE ocupando o resto da tela ----
  if (fullscreen) {
    if (!session) {
      return (
        <div style={{ flex: 1, minHeight: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ width: "min(92vw, 460px)", display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
            {setup}
    ...
    return (
      <div style={{ flex: 1, minHeight: 0, display: "flex", gap: 14, padding: "12px 16px 16px", alignItems: "stretch" }}>
        <aside style={{ width: 330, flex: "none", overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, paddingRight: 4 }}>
          {briefing}
          {error && <div style={{ ...mono, fontSize: 9.5, color: OR }}>⚠ {error}</div>}
        </aside>
        {session.branch ? (
          <LisaPairIDE repo={session.repo} branch={session.branch} onMood={onMood} fill />
        ) : (
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>{noBranchNote}</div>
        )}
      </div>
    );
  }

  // ---- painel do Modo Interativo: tudo empilhado ----
  return (
